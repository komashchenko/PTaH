...
static cell_t PTaH_(IPluginContext* pContext, const cell_t* params)
{
	...
	if (pFunction)
	{
		...
		{
			return g_ForwardManager.UpdateHook(static_cast<PTaH_HookEvent>(params[1]), pFunction, static_cast<bool>(params[2]));
		}

		return pContext->ThrowNativeError("Invalid event PTaH_HookType specified");
	}
